  ...
  context "when signed in" do
    ...
    describe "GET /creators/:id", :as_member do
      ...
      it "serves turbo-stream model windows for infinite scroll" do # rubocop:todo RSpec/MultipleExpectations
        create_list(:model, 6, creator: creator)
        get "/creators/#{creator.to_param}",
      ...
      it "does not leak another creator's models via stream offset" do
        other = create(:creator)
        create_list(:model, 3, creator: creator)
        create(:model, creator: other, name: "secret-other-creator-model")
        get "/creators/#{creator.to_param}",
        ...
        expect(response).to have_http_status(:success)
        expect(response.body).not_to include("secret-other-creator-model")
      end
    end
