class UpdateMetadataFromLinkJob < ApplicationJob
  ...
  def perform(link: nil, link_id: nil, organize: false)
    link ||= Link.find(link_id) if link_id.present?
    return unless link
    ...
    Problems::HttpError.detect(link, should_exist: false)
  rescue Faraday::Error => err
    Rails.logger.error err
    Problems::HttpError.detect(link, should_exist: true, note: err.to_s)
  end
