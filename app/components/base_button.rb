...
class Components::BaseButton < Components::Base
  ...
  VARIANT_CLASSES = begin
    ...
    # Full class string (BASE + secondary) so ApplicationHelper#secondary_action_class is byte-equal.
    variants["toolbar"] = [BASE_CLASSES, variants["secondary"]].join(" ")
    variants.freeze
  end
  ...
  def button_class
    ...
    return variant_classes if variant_classes.start_with?(BASE_CLASSES)
