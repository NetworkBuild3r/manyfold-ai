class CollectionsController < ApplicationController
  ...
  def get_creators
    ...
    @creators = policy_scope(Creator, policy_scope_class: ApplicationPolicy::UpdateScope).order("LOWER(creators.name) ASC")
    @default_creator = @creators.first if @creators.one?
  end

  def get_parent_collections
    # Collection that we can add this one to
    @collections = policy_scope(Collection, policy_scope_class: ApplicationPolicy::UpdateScope).where.not(id: @collection&.id).order("LOWER(collections.name) ASC")
  end
