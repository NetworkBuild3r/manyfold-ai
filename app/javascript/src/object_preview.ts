...
export class ObjectPreview {
  ...
  private ensureProgressUi (container: HTMLElement): void {
    ...
    const canvas = container.querySelector<HTMLElement>('canvas.object-preview')
    const progressLabel = canvas?.dataset.progressAriaLabel ?? ''
    const loadLabel = canvas?.dataset.loadLabel ?? ''
    ...
    bar.className = 'progress-bar h-2 bg-primary-500 rounded overflow-hidden mb-2'
    ...
    label.className = 'progress-label text-sm font-medium block'
    label.setAttribute('role', 'button')
    label.textContent = loadLabel
